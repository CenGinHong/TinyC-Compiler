...
typedef enum
{
  ...
  opLDS,   /* RM    reg(r) = mem(reg[d]+reg(s)) */
  opST,    /* RM     mem(d+reg(s)) = reg(r) */
  opSTS,   /* RM     mem(reg[d]+reg(s)) = reg(r) */
...
STEPRESULT stepTM(void)
{
  ...
  case opLD:
    ...
    break;
  case opLDS:
	reg[r] = dMem[reg[currentinstruction.iarg2] + reg[s]];
    break;
  case opST:
    dMem[m] = reg[r];
    break;
  case opSTS:
    dMem[reg[currentinstruction.iarg2] + reg[s]] = reg[r];
    break;
